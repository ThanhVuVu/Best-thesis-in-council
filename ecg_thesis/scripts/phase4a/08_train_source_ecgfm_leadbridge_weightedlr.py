...
def main() -> None:
    ...
    print(f"Phase 4A weighted-LR fit windows: {len(fit_ds)}, validation windows: {len(val_ds)}")
    output = ensure_dir(cfg_path(config, "paths", "output_dir"))
    summary = train_source_only(fit_ds, val_ds, run_config, output, device)
    write_json(summary, output / "metrics" / "source_only_ecgfm_leadbridge_weightedlr_train_summary.json")
