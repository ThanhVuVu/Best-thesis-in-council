...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", default="outputs/checkpoints/source_only_ecgfm_repeatinitbridge_best.pt")
    ...
    if args.dataset in ("mitbih", "both"):
        ...
        dataset, name = _maybe_subset(dataset, "source_only_ecgfm_repeatinitbridge_mitbih_test", args.max_samples)
        evaluate_and_save(model, dataset, device, output, name, "source_only_ecgfm_repeatinitbridge", wandb_run=wandb_run)

    if args.dataset in ("incart", "both"):
        dataset = ECGBeatDataset(cfg_path(config, "data", "target_test"), return_metadata=True)
        dataset, name = _maybe_subset(dataset, "source_only_ecgfm_repeatinitbridge_incart_heldout", args.max_samples)
        evaluate_and_save(model, dataset, device, output, name, "source_only_ecgfm_repeatinitbridge", wandb_run=wandb_run)
    wandb_run.finish()
