...
def main() -> None:
    ...
    parser.add_argument("--source-checkpoint", default="outputs/checkpoints/source_only_catnet_rr_best.pt")
    parser.add_argument("--dann-checkpoint", default="outputs/checkpoints/dann_rr_best.pt")
    ...
    figures_dir = ensure_dir(output / "figures" / "phase3")
    ...
    source_path = _resolve_checkpoint(config, args.source_checkpoint)
    if source_path.exists():
        source_model, _ = load_model_from_checkpoint(source_path, device)
        source_result = predict_model(source_model, loader, device, collect_embeddings=True, desc="source-only RR embeddings")
        domains = [str(m["domain"]) for m in source_result["metadata"]]
        plot_umap_embeddings(
            source_result["embeddings"],
            source_result["y_true"],
            domains,
            figures_dir / "umap_source_only_catnet_rr.png",
            seed=int(config["seed"]),
            title="Source-only CATNet1D + RR embeddings",
        )

    dann_path = _resolve_checkpoint(config, args.dann_checkpoint)
    if dann_path.exists():
        dann_model, _ = load_dann_from_checkpoint(dann_path, device)
        dann_result = predict_model(dann_model, loader, device, collect_embeddings=True, desc="dann RR embeddings")
        domains = [str(m["domain"]) for m in dann_result["metadata"]]
        plot_umap_embeddings(
            dann_result["embeddings"],
            dann_result["y_true"],
            domains,
            figures_dir / "umap_dann_rr.png",
            seed=int(config["seed"]),
            title="DANN CATNet1D + RR embeddings",
        )

    _plot_training_curves(output / "logs" / "dann_rr_train_log.csv", figures_dir / "training_curves_dann_rr.png")
    _plot_per_class_f1(output / "metrics", figures_dir / "per_class_f1_phase2_phase3.png")
    print(f"Saved Phase 3 figures to {figures_dir}")
...
def _embedding_subset(config: dict, max_points_per_domain: int):
    ...
    source = ECGBeatRRDataset(cfg_path(config, "data", "source_test"), return_metadata=True)
    target = ECGBeatRRDataset(cfg_path(config, "data", "target_test"), return_metadata=True)
    subsets = []
...
def _plot_per_class_f1(metrics_dir: Path, output_path: Path) -> None:
    metric_files = [
        ("P2 Source", metrics_dir / "source_only_catnet_incart_heldout_metrics.json"),
        ("P2 DANN", metrics_dir / "dann_incart_heldout_metrics.json"),
        ("P3 Source RR", metrics_dir / "source_only_catnet_rr_incart_heldout_metrics.json"),
        ("P3 DANN RR", metrics_dir / "dann_rr_incart_heldout_metrics.json"),
    ]
    labels = []
    values = {"N": [], "S": [], "V": []}
