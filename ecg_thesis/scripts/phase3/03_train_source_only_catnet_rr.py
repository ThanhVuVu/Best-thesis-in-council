...
def main() -> None:
    ...
    full_train = ECGBeatRRDataset(cfg_path(config, "data", "source_train"))
    ...
    print(f"Source-only CATNetRR fit beats: {len(fit_ds)}, validation beats: {len(val_ds)}")
    output = ensure_dir(cfg_path(config, "paths", "output_dir"))
    summary = train_source_only(fit_ds, val_ds, run_config, output, device)
    write_json(summary, output / "metrics" / "source_only_catnet_rr_train_summary.json")
