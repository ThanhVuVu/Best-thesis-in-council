...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", default="outputs/checkpoints/source_only_catnet_rr_best.pt")
    ...
    if args.dataset in ("mitbih", "both"):
        dataset = ECGBeatRRDataset(cfg_path(config, "data", "source_test"), return_metadata=True)
        dataset, name = _maybe_subset(dataset, "source_only_catnet_rr_mitbih_test", args.max_samples)
        evaluate_and_save(model, dataset, device, output, name, "source_only_catnet_rr")

    if args.dataset in ("incart", "both"):
        dataset = ECGBeatRRDataset(cfg_path(config, "data", "target_test"), return_metadata=True)
        dataset, name = _maybe_subset(dataset, "source_only_catnet_rr_incart_heldout", args.max_samples)
        evaluate_and_save(model, dataset, device, output, name, "source_only_catnet_rr")
