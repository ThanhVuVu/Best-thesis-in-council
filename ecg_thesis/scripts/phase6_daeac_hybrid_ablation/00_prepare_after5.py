...
def main() -> None:
    ...
    threshold = float(config["data"].get("target_split_seconds", 300.0))
    summary = create_daeac_after_time_split(
        cfg_path(config, "data", "target_full_transductive"),
        cfg_path(config, "data", "target_test"),
        threshold_sec=threshold,
        force=args.force,
    )
    audit = audit_daeac_disjoint(
        cfg_path(config, "data", "target_unlabeled"),
        cfg_path(config, "data", "target_test"),
    )
    if not audit["disjoint"]:
        raise ValueError(f"Target first5/after5 overlap detected: {audit['overlap_count']} samples.")
    output = cfg_path(config, "paths", "output_dir") / "diagnostics" / "after5_prepare.json"
    write_json({"after5": summary, "overlap_audit": audit}, output)
    print({"after5": summary, "overlap_audit": audit})
