...
def main() -> None:
    ...
    run = init_wandb(config, job_type="eval_daeac_hybrid_ablation", default_name=f"{method}_{checkpoint_kind}_eval")
    for dataset_name, path in _datasets(config, args.dataset):
        ds = DAEACDataset(path, input_key=str(config["data"].get("input_key", "auto")), label_key=str(config["data"].get("label_key", "y")), class_names=class_names)
        if dataset_name == "source_val":
            _, ds, _ = split_daeac_source_fit_val(ds)
        ds = subset_first(ds, args.max_samples)
        result = evaluate_daeac_model(model, DataLoader(ds, batch_size=int(config["evaluation"]["batch_size"]), shuffle=False), device, class_names)
        stem = f"{method}_{checkpoint_kind}_{dataset_name}"
        metrics = dict(result["metrics"])
        metrics.update({"dataset": dataset_name, "setting": method, "checkpoint": str(args.checkpoint), "checkpoint_kind": checkpoint_kind})
        write_json(metrics, output / "metrics" / f"{stem}_metrics.json")
        _predictions(output / "predictions" / f"{stem}_predictions.csv", result, class_names)
        _confusion(output / "metrics" / f"{stem}_confusion_matrix.csv", metrics["confusion_matrix"], class_names)
...
def _predictions(path, result, class_names):
    ensure_dir(path.parent)
    with path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.writer(handle)
        writer.writerow(["y_true", "y_pred", "true_class", "pred_class", *[f"prob_{name}" for name in class_names]])
        for true, pred, prob in zip(result["y_true"], result["y_pred"], result["probabilities"]):
            writer.writerow([int(true), int(pred), class_names[int(true)], class_names[int(pred)], *[float(value) for value in np.asarray(prob)]])


def _confusion(path, matrix, class_names):
    ensure_dir(path.parent)
    with path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.writer(handle)
