...
from src.training.train_cdan import load_cdan_from_checkpoint
...
def main() -> None:
    ...
    checkpoint = args.checkpoint or f"outputs/phase2_clef_cdan/checkpoints/{prefix}_best.pt"
    checkpoint_path = cfg_path({"paths": {"checkpoint": checkpoint}, "_base_dir": config["_base_dir"]}, "paths", "checkpoint")
    model, ckpt = load_cdan_from_checkpoint(
        checkpoint_path,
        device,
