...
from src.training.train_cdan import train_cdan
...
def main() -> None:
    ...
    if args.source_init_checkpoint is not None:
        config["cdan"]["source_init_checkpoint"] = args.source_init_checkpoint
    ...
    if args.lambda_base is not None:
        config["cdan"]["lambda_base"] = float(args.lambda_base)
    if args.method is not None:
        config["cdan"]["method"] = args.method
    if args.warmup_epochs is not None:
        config["cdan"]["warmup_epochs"] = int(args.warmup_epochs)
    ...
    print(
        "Phase 2 CLEF-CDAN run config:",
        {
            ...
            "source_init_checkpoint": config["cdan"]["source_init_checkpoint"],
            "method": config["cdan"].get("method"),
            "lambda_base": config["cdan"].get("lambda_base"),
            "warmup_epochs": config["cdan"].get("warmup_epochs"),
            "conditioning": config["cdan"].get("conditioning"),
            "detach_softmax_in_T": config["cdan"].get("detach_softmax_in_T"),
        },
    )
    output = ensure_dir(cfg_path(config, "paths", "output_dir"))
    summary = train_cdan(source_fit, source_val, target, config, output, device)
