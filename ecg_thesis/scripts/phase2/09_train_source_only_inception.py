...
def main() -> None:
    ...
    print(f"Source-only InceptionTime fit beats: {len(fit_ds)}, validation beats: {len(val_ds)}")

    summary = train_source_only(fit_ds, val_ds, run_config, output, device)
    write_json(summary, output / "metrics" / "source_only_inception_train_summary.json")
