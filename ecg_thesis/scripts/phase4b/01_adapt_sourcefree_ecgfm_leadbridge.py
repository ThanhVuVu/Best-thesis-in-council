...
def main() -> None:
    ...
    print(f"Phase 4B target unlabeled windows: {len(target)}")
    ...
    summary = train_source_free(
        ...
        unfreeze_top_ecgfm_layers=int(config["source_free"].get("unfreeze_top_ecgfm_layers", 0)),
        model_kwargs_override={
            "ecgfm_checkpoint_path": args.ecgfm_checkpoint,
            "fairseq_signals_path": args.fairseq_signals_path,
        },
    )
    write_json(summary, output / "metrics" / "sourcefree_ecgfm_leadbridge_train_summary.json")
