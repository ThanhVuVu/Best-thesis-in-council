...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", default="outputs/checkpoints/sourcefree_ecgfm_leadbridge_best.pt")
    ...
    if args.dataset in ("mitbih", "both"):
        ...
        dataset, name = _maybe_subset(dataset, "sourcefree_ecgfm_leadbridge_mitbih_test", args.max_samples)
        evaluate_and_save(model, dataset, device, output, name, "sourcefree_ecgfm_leadbridge", wandb_run=wandb_run)

    if args.dataset in ("incart", "both"):
        dataset = ECGBeatDataset(cfg_path(config, "data", "target_test"), return_metadata=True)
        dataset, name = _maybe_subset(dataset, "sourcefree_ecgfm_leadbridge_incart_heldout", args.max_samples)
        evaluate_and_save(model, dataset, device, output, name, "sourcefree_ecgfm_leadbridge", wandb_run=wandb_run)
    wandb_run.finish()
