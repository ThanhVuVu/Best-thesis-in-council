...
def train_daeac_hybrid_mkmmd(
    ...
    prefix = str(cfg.get("checkpoint_prefix", "daeac_hybrid_mkmmd"))
    ...
    wandb_run = init_wandb(config, job_type="train_daeac_hybrid_mkmmd", default_name=prefix)
    ...
    for epoch in range(int(cfg["epochs"])):
        ...
        history.append(row)
        log_row = {f"hybrid_mkmmd/{k}": v for k, v in row.items() if k not in {"epoch", "pseudo_counts"}}
        for idx, count in enumerate(row["pseudo_counts"]):
            log_row[f"hybrid_mkmmd/pseudo_count_{idx}"] = count
        ...
        print(
            f"[hybrid-mkmmd epoch {epoch + 1}/{cfg['epochs']}] loss={row['loss']:.4f} "
            f"cls={row['loss_cls']:.4f} align={row['loss_align']:.4f} "
            f"sep={row['loss_sep']:.4f} comp={row['loss_comp']:.4f} "
            f"mmd={row['loss_mmd']:.4f} val_macro_f1={row['val_macro_f1']:.4f} "
            f"pseudo={row['pseudo_counts']}"
        )
